...
class ReportGenerator:
    ...
    def _generate_summary_text(self, report: EvaluationReport) -> str:
        ...
        lines = [
            "=" * 60,
            "Medical QA Quality Assessment Report",
            "=" * 60,
            "",
            f"Generated: {report.metadata.get('generated_at', 'N/A')}",
            f"Version: {report.metadata.get('version', 'N/A')}",
            "",
            "-" * 60,
            "Evaluation Summary",
            "-" * 60,
            f"Total Processed: {report.summary.total_processed}",
            f"Retained: {report.summary.retained}",
            f"Discarded: {report.summary.discarded}",
            f"Retention Rate: {report.summary.retention_rate:.1%}",
            f"Average Score: {report.summary.average_score:.1f}",
            "",
            "-" * 60,
            "Evidence Collection Statistics",
            "-" * 60,
            f"Internal Search Triggered: {internal_used} items",
            f"External Search Triggered: {external_used} items",
            "",
            "-" * 60,
